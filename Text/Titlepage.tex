% deprecated: 使用 titlepage-define.tex 和 main.tex 的展示的定义和\maketitle 命令来生成封面
    ...
    \begin{textblock*}{\textwidth}(\dimexpr (\paperwidth - \textwidth) / 2 \relax, 10.6cm)
        {
                ...
                \begin{center}
                    ...
                    \begin{tabular}[b]{B{3cm} B{9.18cm}}
                        {题\hspace{1.1cm}目：} & {福州大学本科生毕业论文} \\\Xcline{2-2}{1pt}       \\[0.24cm]  % 空白行(这个略窄)
                        {}                  & {\LaTeX 模板}   \\  \Xcline{2-2}{1pt}                   \\[0.24cm]  % 空白行
                        {姓\hspace{1.1cm}名：} & {亦瑾}          \\\Xcline{2-2}{1pt}    \\[0.45cm]  % 空白行
                        {学\hspace{1.1cm}号：} & {00000000}    \\\Xcline{2-2}{1pt}    \\[0.45cm]  % 空白行
                        {学\hspace{1.1cm}院：} & {计算机与大数据学院}   \\\Xcline{2-2}{1pt}    \\[0.45cm]  % 空白行
                        {专\hspace{1.1cm}业：} & {计算机科学与技术}    \\\Xcline{2-2}{1pt}    \\[0.45cm]  % 空白行
